<script>
	...
	$: isWebSocket =
		selectedRow?.Metadata?.protocol === "websocket" ||
		selectedRow?.Response?.StatusCode === 101 ||
		selectedRow?.Response?.ContentType === "websocket" ||
		selectedRow?.Metadata?.["websocket.state"];
</script>
	...
	{#if showTitleBar}
		...
		>
			...
			<div class="flex space-x-2 flex-grow px-2">
				...
				>
					<CopyIcon size={14} class="mr-1" /> Copy
					Request
				</button>
				<button
					class="btn btn-sm variant-soft-primary flex items-center"
				...
				>
					<BookCheck size={14} class="mr-1" /> Create
					Test Case
				</button>
				<button
					class="btn btn-sm variant-soft-primary flex items-center"
