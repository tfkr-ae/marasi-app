<script>
	...
	$: {
		...
		requestSearchTimeout = setTimeout(() => {
			const inputFormatted = requestInput
				.toLowerCase()
				.trim();
	...
	$: if (
		...
		testCase.Tags.includes("example_tag")
	) {
		testCase.Tags = testCase.Tags.filter(
			(tag) => tag !== "example_tag",
		);
	}

	$: templateOptions = (
		$marasiConfig?.TestCaseProfile?.TestCases || []
	).map((tc, i) => ({
	...
	function templateFilter() {
		...
		return templateOptions.filter(
			(opt) =>
				opt.Title.toLowerCase().includes(
					inputFormatted,
				) ||
				opt.Category.toLowerCase().includes(
					inputFormatted,
				),
		);
	}
	onDestroy(() => {
		...
		const liveTC = $testCaseStore.find(
...
{#if $modalStore[0]}
	...
	>
		...
		<form
			class="overflow-y-auto grid grid-cols-4 gap-4 custom-scrollbar"
		>
			{#if showTemplateSearch}
				<label
					class="label col-span-4"
					for="Template Search"
				>
			...
			<label class="label col-span-4" for="Requests">
										...
										<button
											...
											on:click|stopPropagation={async () => {
												try {
													...
													toastStore.trigger(
														{
															message: "Failed to unlink request",
															background: "variant-filled-error",
														},
													);
												}
				...
				{:else}
					...
					>
						No requests linked. Search above
						to add one.
					</div>
				{/if}
			</label>
			...
			<label class="label col-span-4" for="Artifacts">
				...
				{:else}
					<div
						class="mt-4 p-3 variant-soft rounded-token text-sm opacity-70 text-center"
					>
						No artifacts uploaded. Drop
						files above to attach them.
					</div>
				{/if}
			</label>
		</form>
