<script>
    ...
    onMount(() => {
        if ($prettify) {
            requestBody = metadata["prettified-request"]
                ? metadata["prettified-request"]
                : request.Raw;
            responseBody = metadata["prettified-response"]
                ? metadata["prettified-response"]
                : response.Raw;
        } else {
    ...
    function viewNote() {
        GetNote(request.ID.toString()).then((note) => {
            const modal = {
                type: "component",
                component: "Notes",
    ...
    <div class="flex-1 p-1 overflow-auto">
        <CodeMirror
            on:change={(event) => {
                ...
                if (
                    !safeCompare(text, metadata["prettified-request"]) &&
