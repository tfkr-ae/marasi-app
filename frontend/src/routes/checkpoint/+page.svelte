<script>
    ...
    import { GetIntercepted, GetInterceptedQueue, CheckHTTPParse, ForwardIntercepted, DropIntercepted, RunExtension, GetInterceptFlag, ToggleIntercept, InterceptResponse } from '../../lib/wailsjs/go/main/App';
    import { onMount } from 'svelte';
    import { EventsOn} from "../../lib/wailsjs/runtime/runtime"
    import { Accordion, AccordionItem, getDrawerStore, getToastStore } from '@skeletonlabs/skeleton';
    import { SettingsIcon } from "svelte-feather-icons";
    import MarasiKeys from "../../lib/components/MarasiMenu/MarasiKeys.svelte";
    import { CornerLeftDown, EditIcon, FileCode, FilePenLine, Forward, SquarePlay, ToggleLeft } from "lucide-svelte";
    import { checkpointCode, marasiConfig} from "../../stores";
    ...
    const checkpointMenu = [
        ...
        },
        {
            ...
            action: {
                handler: () => {
                    ...
                    setTimeout(() => {
                        const cmContent = document.querySelector('div[data-language="lua"]');
                        cmContent.focus();
                    }, 300); 
        ...
        },
        {
            ...
            action: {
                handler: () => {
                    RunExtension("checkpoint", $checkpointCode).then(() => {
                        const toastSettings = {
            ...
            action: {
                handler: () => {
                    if (interceptedCount > 0) {
                        const cmContent = document.querySelector('div[data-language="javascript"]');
                        cmContent.focus();
                    }
                },
                options: {scope: "checkpoint", single: true},
    ...
    function CheckSyntax(text) {
        CheckHTTPParse(text).then((response) => {error = response})
    }
    ...
    function GetNext() {
        ...
        GetInterceptedQueue().then((count) => {interceptedCount = count});
    }
    onMount(() => {
        EventsOn("intercepted", () => GetNext());
        GetNext();
        GetInterceptedQueue().then((count) => {interceptedCount = count});
        GetInterceptFlag().then((flag) => {interceptedFlag = flag});
        return () => {
            //intercepted = "";
...
<Accordion rounded="false">
	<AccordionItem bind:open={accOpened}>
		...
		<svelte:fragment slot="content">
            <CodeMirror bind:value={$checkpointCode} class="text-xs" theme={oneDark} 
...
<div class="btn-group variant-filled-primary">
	<button disabled={error !== ""} on:click={() => {forward(intercepted)}}>Forward</button>
    <button disabled={type !== "request"} on:click = {() => {
        forwardAndInterceptResponse()}}>Intercept Response</button>
    <button class={interceptedFlag ? "btn variant-filled-success hover:variant-filled-success" : "btn variant-filled-primary"} on:click = {() => {
        ToggleIntercept().then((flag) => {
            interceptedFlag = flag
        })}}>{interceptedFlag ? "Global Intercept (On)" : "Global Intercept (Off)"}</button>
...
<CodeMirror bind:value={intercepted} lang={StreamLanguage.define(http)} class="text-xs" theme={oneDark} 
