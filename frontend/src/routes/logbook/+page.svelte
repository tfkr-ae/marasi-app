<script>
    ...
    $: testCases = $testCaseStore
        .filter((tc) => {
            ...
            const descMatch = (tc?.Description ?? "")
                .toLowerCase()
                .includes(query);
            const catMatch = (tc?.Category ?? "").toLowerCase().includes(query);
            const tagMatch = (tc?.Tags ?? []).some((tag) =>
                tag.toLowerCase().includes(query),
            );
            const noteMatch = (tc?.Note ?? "").toLowerCase().includes(query);

            return titleMatch || descMatch || catMatch || tagMatch || noteMatch;
        })
        .sort((a, b) => {
    ...
    $: findings = $findingStore
        .filter((fnd) => {
            ...
            const query = $logbookSearchInput.toLowerCase().trim();
            const titleMatch = (fnd?.Title ?? "").toLowerCase().includes(query);
            ...
            action: {
                handler: () => {
                    if (!$modalStore[0]) {
                        ...
                        setTimeout(() => {
                            const searchBox =
                                document.getElementById("logbookSearch");
            ...
            action: {
                handler: () => {
                    if (!$modalStore[0])
                        findingStore
                            .create([])
                            .then((finding) => openFindingModal(finding, true));
                ...
                handler: () => {
                    if (!$modalStore[0])
                        testCaseStore
                            .create([])
                            .then((testCase) =>
                                openTestCaseModal(testCase, true),
                            );
...
<Accordion rounded="none">
    <AccordionItem bind:open={accOpened}>
        ...
        <svelte:fragment slot="summary">Logbook Settings</svelte:fragment>
        <svelte:fragment slot="content">
            ...
            >
                ...
                >
                    <TabGroup>
                        <Tab bind:group={tabSet} name="combined" value={0}>
                            <div class="flex items-center gap-2">
                                <ListTree size={16} />
                                <span>All</span>
                            </div>
                        </Tab>
                        <Tab bind:group={tabSet} name="findings" value={1}>
                            <div class="flex items-center gap-2">
                                <ShieldAlert size={16} />
                                <span>Findings</span>
                            </div>
                        </Tab>
                        <Tab bind:group={tabSet} name="testcases" value={2}>
                            <div class="flex items-center gap-2">
                                <BookCheck size={16} />
                    ...
                    <div class="flex gap-2">
                        ...
                        >
                            <Plus size={14} class="mr-1" /> Finding
                        </button>
                        <button
                            on:click={() => {
                                if (!$modalStore[0])
                                    ...
                                    $modalStore[0].component === "TestCase"
                                )
                                    modalStore.close();
                            }}
                            class="btn btn-sm variant-filled-tertiary"
                        >
                            <Plus size={14} class="mr-1" /> Test Case
                        </button>
                        <button
                            class="btn btn-sm variant-filled-primary"
                            disabled
                        >
                            <SquareArrowRightIcon size={14} class="mr-1" /> Export
                            Report
                        </button>
                    </div>
                </div>
...
<div class="container p-4">
    {#if (tabSet === 0 || tabSet === 1) && findings.length > 0}
        {#if tabSet === 0}
            <h3 class="h3 font-bold mb-4 flex items-center gap-2">
                <ShieldAlert size={20} class="text-primary-500" />
        ...
        >
            {#each findings as finding (finding.ID)}
                ...
                <div
                    ...
                    on:click={() => openFindingModal(finding, false)}
                >
                    <span
                 ...
                 hover:variant-filled-error shadow-lg"
                        on:click|stopPropagation={() => {
                            modalStore.trigger({
                                ...
                                response: (r) => {
                                    if (r) findingStore.delete(finding.ID);
                                },
                            });
                        }}
                    >
                    ...
                    <header class="card-header">
                        ...
                        <h4 class="h4 font-bold line-clamp-2">
                            {finding?.Title}
                        </h4>
                    </header>
                    <section class="p-2">
                        ...
                        >
                            ...
                            <div
                                class="flex items-center gap-1 {finding?.TestCaseID
                                    ...
                                    : 'opacity-50'}"
                            >
                                <span class="font-bold">Test Case:</span>
                                {finding?.TestCaseID ? "Yes" : "No"}
                            </div>
                            <div class="flex items-center gap-1 opacity-70">
                                <span class="font-bold">Artifacts:</span>
                                {finding?.Artifacts?.length || 0}
                            </div>
                            <div
                                class="flex items-center gap-1 {finding?.TreatmentPlan &&
                                typeof finding.TreatmentPlan === 'string' &&
                                finding.TreatmentPlan.trim()
                                    ? 'text-success-500'
                                    : 'opacity-50'}"
                            >
                                <span class="font-bold">Treatment Plan:</span>
    ...
    {#if (tabSet === 0 || tabSet === 2) && testCases.length > 0}
        {#if tabSet === 0}
            <h3 class="h3 font-bold mb-4 flex items-center gap-2">
                <BookCheck size={20} class="text-tertiary-500" />
        ...
        >
            {#each testCases as testCase (testCase.ID)}
                ...
                <div
                    ...
                    on:click={() => openTestCaseModal(testCase, false)}
                >
                    <span
                 ...
                 hover:variant-filled-warning shadow-lg"
                        on:click|stopPropagation={() => {
                            modalStore.trigger({
                                ...
                                response: (r) => {
                                    if (r) testCaseStore.delete(testCase.ID);
                                },
                            });
                        }}
                    >
                    ...
                    <header class="card-header">
                        ...
                        >
                            <div class="max-w-full">
                                ...
                                {:else}
                                    <span class="badge variant-soft font-bold"
                                        >Uncategorized</span
                                    >
                                {/if}
                            </div>
                    ...
                    <footer class="card-footer">
                        ...
                        >
                            <div class="flex items-center gap-1 opacity-70">
                                <span class="font-bold">Requests:</span>
                                {testCase?.Requests?.length || 0}
                            </div>
                            <div
                                class="flex items-center gap-1 {findings.filter(
                                    (f) => f.TestCaseID === testCase.ID,
                                ).length > 0
                                    ? 'text-tertiary-500'
                                    : 'opacity-50'}"
                            >
