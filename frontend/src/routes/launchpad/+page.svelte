<script>
    ...
    function openMetadata() {
        ...
        const modal = {
            type: "component",
            component: "Metadata",
            content: $proxyItems[currentItem.Request.ID]?.Metadata,
            title:
                ...
                <div class="card p-4 bg-surface-800/30 request-card">
                    {#if currentItem}
                        ...
                        {#key $currentLaunchpadRequestIndex}
                            <RequestResponse
                                requestTitle={"Request " + ($currentLaunchpadRequestIndex + 1)}
                                request={currentItem.Request}
                                response={currentItem.Response}
                                metadata={$proxyItems[currentItem.Request.ID].Metadata}
